...
"""
================================================================================
EXPERIMENT 2: SECONDARY CLASSIFICATION TEST
================================================================================

Tests whether candidate "secondary" variables are harmful or neutral when 
perceived alone (without primary context).
...
HYPOTHESIS:
Secondary variables should be harmful or neutral when perceived alone.
They should only become beneficial when combined with primary context.

CONFIGURATIONS:
- Baseline (6 channels): No extra perception
- env_health (7 ch): Environment health signal
- env_resources (7 ch): Normalized resource level
- obs_sigma (7 ch): Observation noise level
- action_stability (7 ch): Inverse variance of recent actions

METRIC:
Pooled correlation between boundary_std and core_std.
Negative Δ from baseline = harmful when perceived alone.
...
class SecondaryChannelSubstrate(HonestBoundarySubstrate):
    ...
    def _compute_env_resources(self) -> float:
        """Read environment resources (NOT injected - reading existing state)."""
        return float(self.environment.resources / self.environment.max_resources)
    
    def _compute_obs_sigma(self) -> float:
        """Compute observation noise level (NOT injected - derived from state)."""
    ...
    def _compute_action_stability(self) -> float:
        """Compute action stability from recent history (NOT injected)."""
    ...
    def step_physics(self):
        # Track action for stability computation
...
def main():
    print("=" * 70)
    print("EXPERIMENT 2: SECONDARY CLASSIFICATION TEST")
    print("=" * 70)
    print()
    ...
    print()
    print("Hypothesis: Secondary variables are harmful or neutral when alone")
    print()
    ...
    if abs(env_health_r - env_resources_r) < 0.05:
        print("\n  ✓ env_health ≡ env_resources (same effect)")
    else:
        print("\n  × env_health ≢ env_resources (different effects)")
    
    if abs(env_health_r - obs_sigma_r) < 0.05:
        print("  ✓ obs_sigma ≡ env_health (same effect)")
    else:
        print("  × obs_sigma ≢ env_health (different effect)")
